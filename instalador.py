...
conexao = MySQLdb.connect(host='localhost' , user='snep' , passwd='sneppass', db='snep')
cursor = conexao.cursor()
cursor.execute('''CREATE TABLE IF NOT EXISTS `snep`.`rfid` (
...
cursor.execute('''CREATE TABLE IF NOT EXISTS `snep`.`senha` (
  `id` INT NOT NULL AUTO_INCREMENT,
...
cursor.execute('''CREATE TABLE IF NOT EXISTS `snep`.`tb_porteiro` (
  `id` INT NOT NULL AUTO_INCREMENT,
...
cursor.execute('''CREATE TABLE IF NOT EXISTS `snep`.`tb_grupos` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `grupo` VARCHAR(100) NOT NULL,
  `cadastro` VARCHAR(50) NOT NULL,
  `atualizado` VARCHAR(50) NOT NULL,
...
cursor.execute('''CREATE TABLE IF NOT EXISTS `snep`.`tb_porteirogrupos` (
